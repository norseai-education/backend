...
class Teacher:
    ...
    def store_input(self, student_id, student_input):
        """Store student input in background without blocking the main flow"""
        utils.log("Storing student input to student_persona collection...", logger, 2)
        def background_task():
            try:
                # Try to use existing event loop if available
                loop = asyncio.get_running_loop()
                asyncio.create_task(
                    self.store.insert_document('student_persona', {"student_id": str(student_id), "content": student_input})
                )
            except RuntimeError:
    ...
    def build_node(self, state: State):
        ...
        utils.log(f"Going through teacher node...", logger, 2)
        ...
        self.store_input(student_id, student_input)
        ...
        utils.log("Getting teacher prompt", logger, 2)
        ...
        utils.log("Getting teacher response...", logger, 2)
