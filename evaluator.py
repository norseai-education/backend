...
class EvaluatorModel:
    ...
    def format_eval_output(self, eval_result):
            ...
            if "Evaluation of Concepts" and "Solution" in eval_result.keys():
                ...
                if not isinstance(eval_grade, dict):
                    try:
                        ...
                        utils.log("Failed to parse eval grade JSON", logger, 0)
                        eval_grade = {}
        ...
        for key in eval.keys():
            if key not in utils.concepts:
                del eval_grade[key]
        if solution:
            return eval_grade, solution
        else:
            utils.log("Failed to find eval solution", logger, 0)
            return eval_grade, ""

    def build_node(self, state: State): 
        utils.log(f"Current state: {state}", logger, 2)
        utils.log(f"Going through evaluator node...", logger, 2)
