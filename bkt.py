...
from utils import knowledge_graph
import utils
...
class BayesianKnowledgeTracing():
    ...
    def bkt_algorithm(self, grade, cur_graph, damping_factor):
        utils.log(f"Used damping factor of {damping_factor}", logger, 2)
    ...
    def update_learning_obj(self, cur_learning_obj, init_learning_obj, updated_graph, learning_status):
        utils.log("Updating learning objectives and learning status...", logger, 2)
        ...
        if concepts_behind:
            ...
            utils.log(f"Concept behind found: \n{earliest_behind}", logger, 2)
        ...
        if current_mastery > 0.95:
            utils.log("Current mastery is above 0.95!", logger, 2)
            ...
            if current_status == "behind":
                ...
                if remaining_behind:
                    ...
                    utils.log("Returning to original objective!", logger, 2)
                    return {
            ...
            if next_index < len(self.list_of_learning_obj):
                ...
                utils.log("Moving ahead to next objective!", logger, 2)
                return {
    ...
    def build_node(self, state: State):
        ...
        utils.log("Going through BKT node...", logger, 2)
        ...
        utils.log(f"Updated bkt graph: \n{updated_graph}", logger, 2)
        cur_mastery = self.get_mastery(updated_graph)
        utils.log(f"Student's current knowledge: {cur_mastery}", logger, 2)
